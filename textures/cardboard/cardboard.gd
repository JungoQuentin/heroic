...
func _compute_face_uv(p: Vector2, minv: Vector2, size: Vector2) -> Vector2:
	# UV 0..1 dans la bounding box du polygone
	# (le 1.0 - ... sur Y pour matcher le sens habituel des UV)
	return Vector2(
		(p.x - minv.x) / size.x,
...
func regenerate():
	...
	# Calcul périmètre pour UV U (tiling propre le long du contour)
	var perim := 0.0
	for i in poly.size():
	...
	for i in poly.size():
		...
		# 4 coins du quad extrudé
		var a0 := Vector3(p0.x, p0.y, zf)
		var a1 := Vector3(p1.x, p1.y, zf)
		var b1 := Vector3(p1.x, p1.y, zb)
		var b0 := Vector3(p0.x, p0.y, zb)

		# Normale approx (perpendiculaire à l’edge dans le plan XY)
		# Si tu vois la tranche "à l'envers", inverse n2 (ou inverse l’ordre des points du polygone)
		var e := (p1 - p0).normalized()
		var n2 := Vector2(-e.y, e.x)
		var n := Vector3(n2.x, n2.y, 0).normalized()

		# UV: U le long, V sur la profondeur
